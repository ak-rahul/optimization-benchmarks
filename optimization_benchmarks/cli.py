...
def print_function_list():
    ...
    for name in get_all_functions():
        if name in BENCHMARK_SUITE:
            ...
            print(f"{name:<25} | {meta['default_dim']:3d} | {meta['known_minimum']:12.4f} | {bounds_str}")
        else:
...
def main():
    ...
    parser.add_argument('--list', action='store_true',
                       help="List all available functions with metadata")
    parser.add_argument('--info', metavar='FUNCTION',
                       help="Show documentation and metadata for the specified function")
    parser.add_argument('--metadata', metavar='FUNCTION',
    ...
    parser.add_argument('--input', metavar='FILE',
                       help="CSV file with input vectors for batch evaluation")
    parser.add_argument('--output', metavar='FILE',
                       help="Output file to write results in JSON format")
