...
from optimization_benchmarks.benchmarking import (
    BenchmarkRunner,
    quick_benchmark
)
...
def simple_optimizer(func, bounds, max_iter=100):
    ...
    for _ in range(max_iter):
        x = bounds_array[:, 0] + np.random.rand(len(bounds)) * (bounds_array[:, 1] - bounds_array[:, 0])
        cost = func(x)
...
def test_benchmark_runner_suite():
    runner = BenchmarkRunner(simple_optimizer, algorithm_name='TestAlgo', 
...
def test_quick_benchmark():
    results = quick_benchmark(simple_optimizer, function_names=['sphere', 'ackley'], 
                             n_runs=2, max_iter=50)
    assert len(results) == 2
